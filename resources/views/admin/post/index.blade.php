    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin#postcreate') }}" method="post" enctype="multipart/form-data">
                ...
                <input type="text" name="postTitle" class="form-control" value="{{ old('postTitle') }}">
                @error('postTitle')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
                <label class="mt-3">Description</label>
                <textarea name="PostDescription" cols="30" rows="5" class="form-control">{{ old('PostDescription') }}</textarea>
                @error('PostDescription')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
                <label class="mt-3">Post Image</label>
