    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin#categoryupdate',$updateCategory['category_id']) }}" method="post">
                ...
                <input type="text" name="categoryTitle" class="form-control" value="{{ old('categoryTitle',$updateCategory['title']) }}">
                @error('categoryTitle')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
                <label class="mt-3">Description</label>
                <textarea name="categoryDescription" cols="30" rows="5" class="form-control">{{ old('categoryTitle',$updateCategory['description']) }}</textarea>
                ...
                <a href="{{ route('admin#category') }}" class="text-secondary" style="margin:0 130px">Go To Create <i class="fa-solid fa-arrow-right ms-1 mt-2"></i> </a>
            </form>
        </div>
    </div>
</div>
